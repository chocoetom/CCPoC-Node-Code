...
import { Address, BIGINT_0, BIGINT_31, BIGINT_32, KECCAK256_NULL, equalsBytes, } from '@ethereumjs/util';
import { activeCostPerStateByte } from "../eip8037.js";
import { EVMError } from "../errors.js";
import { accessAddressEIP2929, addAddressToBAL, getAddressAccessCost, warmAddress, } from "./EIP2929.js";
import { maxCallGas, subMemUsage, trap, writeCallOutput } from "./util.js";
/** True when `regularGas + stateGas` fits in `gas_left + state_gas_reservoir` (EIP-8037). */
export function canAfford(runState, regularGas, stateGas = BIGINT_0) {
...
export function newAccountStateGasCost(common, runState) {
 ...
 */
export async function createNewAccountStateGasIfCharged(runState, common, targetAddress) {
    if (!common.isActivatedEIP(8037)) {
        return BIGINT_0;
    }
...
export function eip7928TrapPreTarget(runState, common, gas) {
...
export function eip7928SchedulePostTargetCallOog(runState, outOffset, outLength) {
...
export function eip7928PostTargetCreateOog(_runState, _common, _gas) {
    trap(EVMError.errorMessages.OUT_OF_GAS);
}
/** Post-target CALL OOG when 7928 is off (legacy: trap in gas handler). */
function postTargetCallOogOrTrap(runState, common, outOffset, outLength) {
    if (!common.isActivatedEIP(7928)) {
        trap(EVMError.errorMessages.OUT_OF_GAS);
    }
    return eip7928SchedulePostTargetCallOog(runState, outOffset, outLength);
}
export function consumeEip7928PostTargetCallOog(runState) {
...
export async function getCallNewAccountPostCosts(runState, common, toAddress) {
...
function commitTargetAccess(runState, toAddress, common) {
    if (common.isActivatedEIP(2929)) {
        warmAddress(runState, toAddress.bytes);
    }
    addAddressToBAL(runState, toAddress.bytes, common);
}
function finalizeCallMessageGas(runState, common, gas, currentGasLimit, newAccountStateGas, outOffset, outLength) {
 ...
 */
export async function callFamilyGas(runState, gas, common, opts) {
    const { currentGasLimit, toAddress, value, inOffset, inLength, outOffset, outLength, includeValueTransfer, includeNewAccountPostCheck, eip7702GetAccessCost, eip7702WarmAddress, } = opts;
    ...
    if (common.isActivatedEIP(7702)) {
        const { gas: delegationGas, delegationAddress } = await eip7702GetAccessCost(runState, common, toAddress, charge2929Gas);
        gas += delegationGas;
    ...
    return finalizeCallMessageGas(runState, common, gas, currentGasLimit, newAccountStateGas, outOffset, outLength);
}
/**
 * Shared EIP-7928 / EIP-8037 gas path for CREATE and CREATE2.
 */
export async function create7928Gas(runState, gas, common, opts, getCreateTargetAddressBytes) {
    ...
    const newAccountStateGas = await createNewAccountStateGasIfCharged(runState, common, targetAddress);
...
//# sourceMappingURL=EIP7928.js.map