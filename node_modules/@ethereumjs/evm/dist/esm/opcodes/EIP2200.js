 ...
 */
export function chargeSstoreAccessEIP8038(runState, key, common) {
 ...
 */
export function updateSstoreGasEIP8038(runState, currentStorage, originalStorage, value, _key, common) {
    ...
    if (!currentEqualsNew) {
        if (originalStorage.length > 0 && currentStorage.length > 0 && value.length === 0) {
            ...
            runState.interpreter.refundGas(common.param('refundStorageClearGas'), 'EIP-8038 -> refundStorageClear');
        }
        if (originalStorage.length > 0 && currentStorage.length === 0) {
            // Gas refund issued earlier to be reversed
            runState.interpreter.subRefund(common.param('refundStorageClearGas'), 'EIP-8038 -> refundStorageClear (reversed)');
        }
        if (originalEqualsNew) {
            ...
            runState.interpreter.refundGas(common.param('storageWriteGas'), 'EIP-8038 -> storageWrite restore refund');
        }
    }
    return gas;
}
