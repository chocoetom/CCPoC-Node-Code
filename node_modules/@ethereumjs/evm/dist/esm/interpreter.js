...
import { Account, Address, BIGINT_0, BIGINT_1, BIGINT_2, EthereumJSErrorWithoutCode, KECCAK256_NULL, KECCAK256_RLP, MAX_UINT64, bigIntToBytes, bigIntToHex, bytesToBigInt, bytesToHex, equalsBytes, generateAddress, generateAddress2, setLengthLeft, setLengthRight, } from '@ethereumjs/util';
import debugDefault from 'debug';
import { EIP7708_BURN_TOPIC, EIP7708_SYSTEM_ADDRESS, EIP7708_TRANSFER_TOPIC } from "./eip7708.js";
...
import { consumeEip7928PostTargetCallOog, } from "./opcodes/EIP7928.js";
import { isEIP8024PairImmediateValid, isEIP8024SingleImmediateValid } from "./opcodes/EIP8024.js";
import { trap } from "./opcodes/index.js";
import { Stack } from "./stack.js";
import { stackDelta } from "./eof/stackDelta.js";
const debugGas = debugDefault('evm:gas');
function isEVMError(error) {
...
export class Interpreter {
    ...
    chargeStateGas(amount, context) {
        ...
        if (evm.DEBUG) {
            debugGas(`${context !== undefined ? context + ': ' : ''}charged ${amount} state gas (reservoir=${evm.stateGasReservoir}, executionStateGasUsed=${evm.executionStateGasUsed}, gasLeft=${this._runState.gasLeft}, spilled=${this._runState.stateGasSpilled})`);
        }
    }
    /**
     ...
     * @param context - Usage context for debugging
     */
    creditStateGasRefund(amount, context) {
        if (amount === BIGINT_0)
            return;
        const evm = this._evm;
        ...
        evm.executionStateGasUsed -= amount;
        if (evm.DEBUG) {
            debugGas(`${context !== undefined ? context + ': ' : ''}credited ${amount} state gas refund (gasLeft +${fromGasLeft}, reservoir=${evm.stateGasReservoir}, executionStateGasUsed=${evm.executionStateGasUsed})`);
        }
    }
    /**
    ...
    async create(gasLimit, value, codeToRun, salt, eofCallData) {
        ...
        const refundCreatePreCharge = (context) => {
        ...
        if (this.common.isActivatedEIP(8037)) {
            if (results.execResult.exceptionError !== undefined) {
                ...
                const haltedOntoAlive = results.execResult.exceptionError.error !== EVMError.errorMessages.REVERT &&
                    message.createdTargetAlive === true;
                if (haltedOntoAlive) {
                    const preCharged = this._runState.lastCreateNewAccountStateGas ?? BIGINT_0;
                    if (preCharged > BIGINT_0) {
                        const fromSpill = preCharged < this._runState.stateGasSpilled
                            ? preCharged
    ...
    async _selfDestruct(toAddress) {
        ...
        if (this.common.isActivatedEIP(7708) &&
            ...
            const data = setLengthLeft(bigIntToBytes(contractBalance), 32);
            const burnLog = [EIP7708_SYSTEM_ADDRESS, [EIP7708_BURN_TOPIC, contractTopic], data];
