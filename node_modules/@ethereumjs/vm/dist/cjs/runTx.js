 ...
 */
async function processAuthorizationList(vm, tx, caller, initialGasRefund, block, gasLimit) {
    ...
    const preDelegatedByAuthority = new Map();
    const accountLeafCharged = new Set();
    const indicatorCharged = new Set();
    const accountWritePaid = new Set([caller.toString()]);
    if (tx.to !== undefined && tx.value > util_1.BIGINT_0) {
        accountWritePaid.add(tx.to.toString());
    }
    const is8037 = tx.common.isActivatedEIP(8037);
    const chargeState = (amount) => {
        ...
        if (vm.evm.stateGasReservoir >= amount) {
            ...
            vm.evm.stateGasReservoir = util_1.BIGINT_0;
            gasLimit -= remainder;
        }
        else {
            return false;
    ...
    for (let i = 0; i < authorizationList.length; i++) {
        ...
        if (account.nonce !== (0, util_1.bytesToBigInt)(authorityNonce)) {
            applyInvalidAuthRefund();
            continue;
        }
        ...
        const codeBeforeAuth = vm.common.isActivatedEIP(7928) || vm.common.isActivatedEIP(8037)
        ...
        if (is8037) {
            ...
            const costPerStateByte = (0, evm_1.activeCostPerStateByte)(vm.common, block?.header.gasLimit);
            const authorityHex = authority.toString();
            const curDelegated = codeBeforeAuth !== undefined &&
                codeBeforeAuth.length >= 3 &&
                (0, util_1.equalsBytes)(codeBeforeAuth.slice(0, 3), DELEGATION_7702_FLAG);
            ...
            const clearing = (0, util_1.equalsBytes)(data[1], new Uint8Array(20));
            ...
            let chargeStateBytes = util_1.BIGINT_0;
        ...
        const currentCode = vm.common.isActivatedEIP(7928) || vm.common.isActivatedEIP(8037) ? codeBeforeAuth : undefined;
...
async function processSelfdestructs(vm, results) {
    ...
    const finalBalancesForBAL = new Map();
        ...
        if (vm.common.isActivatedEIP(8246)) {
            ...
            if ((await vm.stateManager.getAccount(address)) !== undefined) {
                ...
                if (account !== undefined) {
                    account.nonce = util_1.BIGINT_0;
                    await vm.evm.journal.putAccount(address, account);
                }
                finalBalancesForBAL.set(address.toString(), account?.balance ?? util_1.BIGINT_0);
            }
            else {
                finalBalancesForBAL.set(address.toString(), util_1.BIGINT_0);
            }
        }
        else {
            if (vm.common.isActivatedEIP(7708)) {
                ...
                if (finalizationBalance > util_1.BIGINT_0) {
                    finalizationLogs.push((0, evm_1.createEIP7708BurnLog)(address, finalizationBalance));
                }
            }
    ...
    if (destroyedForBAL.size > 0 && vm.common.isActivatedEIP(7928)) {
        vm.evm.blockLevelAccessList.cleanupSelfdestructed([...destroyedForBAL], vm.common.isActivatedEIP(8246) ? finalBalancesForBAL : undefined);
    }
}
/**
...
async function _runTx(vm, opts) {
    ...
    if (tx.value < util_1.BIGINT_0) {
        throw (0, util_1.EthereumJSErrorWithoutCode)(`value field cannot be negative, received ${tx.value}`);
    }
    ...
    const floorCost = (0, tx_1.getCalldataFloorGas)(tx, caller);
    // ===========================
    ...
    // ===========================
    let stateGasReservoirInitial = util_1.BIGINT_0;
    vm.evm.eip7928CallPostTargetOog = false;
    const { intrinsicRegular: intrinsicRegularGas, intrinsicState: intrinsicStateGas } = (0, evm_1.computeIntrinsicGasDimensions8037)(tx.common, tx, block?.header.gasLimit, caller);
    const totalIntrinsic = intrinsicRegularGas + intrinsicStateGas;
    let gasLimit = tx.gasLimit;
    const minGasLimit = (0, util_1.bigIntMax)(totalIntrinsic, floorCost);
    if (gasLimit < minGasLimit) {
        const msg = _errorMsg(`INTRINSIC_GAS_TOO_LOW: tx gas limit ${Number(gasLimit)} is lower than the minimum gas limit of ${Number(minGasLimit)}`, vm, block, tx);
        throw (0, util_1.EthereumJSErrorWithoutCode)(msg);
    }
    if (vm.common.isActivatedEIP(8037)) {
        ...
        const regularCapCheck = (0, util_1.bigIntMax)(intrinsicRegularGas, floorCost);
        if (regularCapCheck > txMaxGasLimit) {
            const msg = _errorMsg(`Transaction intrinsic regular gas ${regularCapCheck} exceeds the EIP-7825 cap (${txMaxGasLimit})`, vm, block, tx);
            throw (0, util_1.EthereumJSErrorWithoutCode)(msg);
        }
        const executionGas = gasLimit - totalIntrinsic;
        const regularBudget = txMaxGasLimit > intrinsicRegularGas ? txMaxGasLimit - intrinsicRegularGas : util_1.BIGINT_0;
    ...
    if (!(0, util_1.equalsBytes)(fromAccount.codeHash, util_1.KECCAK256_NULL)) {
    ...
    fromAccount.nonce += util_1.BIGINT_1;
    await vm.evm.journal.putAccount(caller, fromAccount);
    if (vm.common.isActivatedEIP(7928)) {
    ...
    if (vm.common.isActivatedEIP(8037)) {
        ...
        vm.evm.executionStateGasUsed = util_1.BIGINT_0;
        vm.evm.eip2780PrepOog = false;
        vm.evm._stateGasSnapshots = [];
        vm.evm.createdAccountStateGas =
            new Map();
        vm.evm.createdAccountIntrinsicStateGas = new Map();
    }
    ...
    if (tx.supports(tx_1.Capability.EIP7702EOACode)) {
        const result = await processAuthorizationList(vm, tx, caller, gasRefund, block, gasLimit);
    ...
    if (authOog) {
        ...
        results = {
            execResult: {
                ...
                exceptionError: new evm_1.EVMError(evm_1.EVMError.errorMessages.OUT_OF_GAS),
            },
        };
    }
    else {
    ...
    const txCreateIntrinsicStateGasRefund = util_1.BIGINT_0;
    ...
    if (vm.common.isActivatedEIP(8037) &&
        results.execResult.exceptionError?.error === evm_1.EVMError.errorMessages.OUT_OF_GAS &&
    ...
    if (vm.common.isActivatedEIP(8037)) {
        ...
        const txStateGasRaw = intrinsicStateGas + executionStateGasUsed - txCreateIntrinsicStateGasRefund;
        const txStateGasClamped = txStateGasRaw > util_1.BIGINT_0 ? txStateGasRaw : util_1.BIGINT_0;
        results.txStateGas = txStateGasClamped;
        const txRegularGasRaw = totalGasSpentBeforeRefund - txStateGasClamped;
        results.txRegularGas = (0, util_1.bigIntMax)(txRegularGasRaw, floorCost);
    }
