...
async function runBlock(vm, opts) {
    ...
    let providedBlockAccessList;
    if (vm.common.isActivatedEIP(7928) && opts.blockAccessList !== undefined) {
        providedBlockAccessList = parseProvidedBlockAccessList(opts.blockAccessList, block.header.blockAccessListHash, generateFields);
        (0, util_1.validateBlockAccessListStructure)(providedBlockAccessList);
    }
    if (vm.DEBUG) {
        debug('-'.repeat(100));
    ...
    if (block.common.isActivatedEIP(7685)) {
        ...
        try {
            requests = await (0, requests_ts_1.accumulateRequests)(vm, result.results, block.header.gasLimit, generateFields);
        }
        catch (err) {
    ...
    if (vm.common.isActivatedEIP(7928) && vm.evm.blockLevelAccessList !== undefined) {
        try {
            (0, util_1.validateBlockAccessListGasLimit)(vm.evm.blockLevelAccessList, block.header.gasLimit);
        }
        catch {
            await vm.evm.journal.revert();
            if (vm.DEBUG) {
        ...
        try {
            ...
            if (vm.common.isActivatedEIP(7928) && providedBlockAccessList !== undefined) {
                ...
                if (generated === undefined ||
                    (0, util_1.equalsBlockAccessList)(providedBlockAccessList, generated) === false) {
                    if (vm.DEBUG) {
                        debug(`Invalid block access list received hash=${(0, util_1.bytesToHex)(providedBlockAccessList.hash())} expected hash=${(0, util_1.bytesToHex)(generated?.hash() ?? new Uint8Array(32))}`);
                    }
...
async function applyTransactions(vm, block, opts) {
    ...
    let blockRegularGasUsed = util_1.BIGINT_0;
    let blockStateGasUsed = util_1.BIGINT_0;
    ...
    for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
        ...
        if (vm.common.isActivatedEIP(8037)) {
            if ((0, evm_1.txExceedsAvailableBlockGas8037)(tx.gasLimit, tx.common.param('maxTransactionGasLimit'), block.header.gasLimit, blockRegularGasUsed, blockStateGasUsed)) {
        ...
        if (vm.common.isActivatedEIP(8037) && txRes.txRegularGas !== undefined) {
            ...
            blockStateGasUsed += txRes.txStateGas ?? util_1.BIGINT_0;
...
function parseProvidedBlockAccessList(blockAccessList, blockAccessListHash, generateFields) {
    if (blockAccessList instanceof util_1.BlockLevelAccessList) {
        if (blockAccessListHash !== undefined && generateFields === false) {
            (0, util_1.validateBlockAccessListHash)(blockAccessList, blockAccessListHash);
        }
        return blockAccessList;
    }
    if (blockAccessList instanceof Uint8Array) {
        const bal = (0, util_1.createBlockLevelAccessListFromRLP)(blockAccessList);
        if (blockAccessListHash !== undefined && generateFields === false) {
            (0, util_1.validateBlockAccessListHash)(bal, blockAccessListHash);
        }
        return bal;
    }
    const json = blockAccessList;
    (0, util_1.validateBlockAccessListJSONStructure)(json);
    if ((0, util_1.isAccountOrderOnlyViolation)(json) === true) {
        throw (0, util_1.EthereumJSErrorWithoutCode)('invalid header: block access list accounts are not sorted');
    }
    if (blockAccessListHash !== undefined && generateFields === false) {
        (0, util_1.validateBlockAccessListHashFromJSON)(json, blockAccessListHash);
    }
    return (0, util_1.createBlockLevelAccessListFromJSON)(json);
}
const DAOConfig = {
    DAOAccounts: [
