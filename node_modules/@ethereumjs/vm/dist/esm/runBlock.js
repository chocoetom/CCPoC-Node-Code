...
import { Account, Address, BIGINT_0, BIGINT_1, BIGINT_8, BlockLevelAccessList, EthereumJSErrorWithoutCode, GWEI_TO_WEI, KECCAK256_RLP, bigIntToAddressBytes, bigIntToBytes, bytesToHex, concatBytes, createAddressFromString, createBlockLevelAccessList, createBlockLevelAccessListFromJSON, createBlockLevelAccessListFromRLP, equalsBlockAccessList, equalsBytes, hexToBytes, intToBytes, isAccountOrderOnlyViolation, setLengthLeft, short, unprefixedHexToBytes, validateBlockAccessListGasLimit, validateBlockAccessListHash, validateBlockAccessListHashFromJSON, validateBlockAccessListJSONStructure, validateBlockAccessListStructure, } from '@ethereumjs/util';
        ...
        try {
            ...
            if (vm.common.isActivatedEIP(7928) && providedBlockAccessList !== undefined) {
                ...
                if (generated === undefined ||
                    ...
                    if (vm.DEBUG) {
                        debug(`Invalid block access list received hash=${bytesToHex(providedBlockAccessList.hash())} expected hash=${bytesToHex(generated?.hash() ?? new Uint8Array(32))}`);
                    }
...
async function applyTransactions(vm, block, opts) {
    ...
    for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
        ...
        if (vm.common.isActivatedEIP(8037)) {
            if (txExceedsAvailableBlockGas8037(tx.gasLimit, tx.common.param('maxTransactionGasLimit'), block.header.gasLimit, blockRegularGasUsed, blockStateGasUsed)) {
