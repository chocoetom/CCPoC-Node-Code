...
import { BIGINT_0, CLRequest, CLRequestType, EthereumJSErrorWithoutCode, bigIntToAddressBytes, bigIntToBytes, bytesToBigInt, bytesToHex, concatBytes, createAddressFromString, setLengthLeft, } from '@ethereumjs/util';
/**
 ...
 */
export const accumulateRequests = async (vm, txResults, blockGasLimit, generate = false) => {
    const requests = [];
    ...
    if (common.isActivatedEIP(8282)) {
        const builderDepositsRequest = await accumulateBuilderRequest(vm, CLRequestType.BuilderDeposit, 'builderDepositContractAddress', blockGasLimit, generate);
        requests.push(builderDepositsRequest);
        const builderExitsRequest = await accumulateBuilderRequest(vm, CLRequestType.BuilderExit, 'builderExitContractAddress', blockGasLimit, generate);
...
const accumulateBuilderRequest = async (vm, requestType, contractAddressParam, blockGasLimit, generate = false) => {
    ...
    if (contractCode.length === 0) {
        ...
        throw EthereumJSErrorWithoutCode(`system contract empty: no code at builder request contract ${builderAddress}`);
    }
    ...
    if (results.execResult.exceptionError !== undefined) {
        ...
        throw EthereumJSErrorWithoutCode(`system contract call failed: builder request contract ${builderAddress} (${results.execResult.exceptionError.error})`);
    }
