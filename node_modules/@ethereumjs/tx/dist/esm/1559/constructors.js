...
import { EthereumJSErrorWithoutCode, bytesToBigInt, bytesToHex, equalsBytes, validateNoLeadingZeroes, } from '@ethereumjs/util';
 ...
 */
export function createFeeMarket1559Tx(txData, opts = {}) {
 ...
 */
export function create1559FeeMarketTxFromBytesArray(values, opts = {}) {
    if (values.length !== 9 && values.length !== 12) {
        throw EthereumJSErrorWithoutCode('Invalid EIP-1559 transaction. Only expecting 9 values (for unsigned tx) or 12 values (for signed tx).');
    }
 ...
 */
export function createFeeMarket1559TxFromRLP(serialized, opts = {}) {
    if (equalsBytes(serialized.subarray(0, 1), txTypeBytes(TransactionType.FeeMarketEIP1559)) === false) {
        throw EthereumJSErrorWithoutCode(`Invalid serialized tx input: not an EIP-1559 transaction (wrong tx type, expected: ${TransactionType.FeeMarketEIP1559}, received: ${bytesToHex(serialized.subarray(0, 1))}`);
    }
    const values = RLP.decode(serialized.subarray(1));
    if (!Array.isArray(values)) {
        throw EthereumJSErrorWithoutCode('Invalid serialized tx input: must be array');
    }
    return create1559FeeMarketTxFromBytesArray(values, opts);
}
