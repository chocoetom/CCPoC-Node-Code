 ...
 */
export function createAccessList2930Tx(txData, opts = {}) {
 ...
 */
export function createAccessList2930TxFromBytesArray(values, opts = {}) {
    if (values.length !== 8 && values.length !== 11) {
        throw EthereumJSErrorWithoutCode('Invalid EIP-2930 transaction. Only expecting 8 values (for unsigned tx) or 11 values (for signed tx).');
    }
 ...
 */
export function createAccessList2930TxFromRLP(serialized, opts = {}) {
    if (equalsBytes(serialized.subarray(0, 1), txTypeBytes(TransactionType.AccessListEIP2930)) === false) {
        throw EthereumJSErrorWithoutCode(`Invalid serialized tx input: not an EIP-2930 transaction (wrong tx type, expected: ${TransactionType.AccessListEIP2930}, received: ${bytesToHex(serialized.subarray(0, 1))}`);
    }
