...
const compareStorageSlotKeys_ts_1 = require("./compareStorageSlotKeys.js");
...
function validateBlockAccessListJSONAccount(account) {
    ...
    for (const slotChange of account.storageChanges) {
        if (prevSlot !== undefined && (0, compareStorageSlotKeys_ts_1.compareStorageSlotKeys)(prevSlot, slotChange.slot) >= 0) {
    ...
    for (const slot of account.storageReads) {
        if (prevRead !== undefined && (0, compareStorageSlotKeys_ts_1.compareStorageSlotKeys)(prevRead, slot) >= 0) {
...
function validateStorageChanges(storageChanges) {
    ...
    for (const [slot, changes] of storageChanges) {
        if (prevSlot !== undefined && (0, compareStorageSlotKeys_ts_1.compareStorageSlotKeys)(prevSlot, slot) >= 0) {
            throwInvalidBlockAccessList('storage slots are not sorted');
        }
        prevSlot = slot;
...
function validateStorageReads(storageReads, storageChanges) {
    ...
    for (const slot of storageReads) {
        ...
        if (prevRead !== undefined && (0, compareStorageSlotKeys_ts_1.compareStorageSlotKeys)(prevRead, slotHex) >= 0) {
            throwInvalidBlockAccessList('storage reads are not sorted');
        }
