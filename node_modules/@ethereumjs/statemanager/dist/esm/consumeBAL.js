import { EthereumJSErrorWithoutCode, KECCAK256_NULL, bytesToHex, createAddressFromString, equalsBytes, hexToBigInt, hexToBytes, setLengthLeft, } from '@ethereumjs/util';
/**
 ...
 */
export async function consumeBAL(stateManager, bal, expectedStateRoot) {
    for (const acc of bal) {
        ...
        const nonce = lastNonceChange?.postNonce !== undefined ? hexToBigInt(lastNonceChange.postNonce) : undefined;
        ...
        if (finalBalance === 0n &&
            ...
            for (const storage of acc.storageChanges) {
                ...
                await stateManager.putStorage(address, setLengthLeft(hexToBytes(storage.slot), 32), setLengthLeft(hexToBytes(value), 32));
            }
        }
    }
    if (expectedStateRoot !== undefined) {
        ...
        if (!equalsBytes(expectedStateRoot, stateRoot)) {
            throw EthereumJSErrorWithoutCode(`Expected state root ${bytesToHex(expectedStateRoot)} but got ${bytesToHex(stateRoot)}`);
        }
    }
}
