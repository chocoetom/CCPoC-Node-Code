...
class MerklePatriciaTrie {
    ...
    async del(key, skipKeyTransform = false) {
        ...
        const nodeHasValue = node !== null && (!(node instanceof index_ts_1.BranchMPTNode) || node.value() !== null);
        let ops = [];
    ...
    async _deleteNode(keyBytes, stack) {
        ...
        if (branchNodes.length === 0) {
            ...
            if (branchValue !== null) {
                ...
                const leafNode = new index_ts_1.LeafMPTNode([], branchValue);
                if (parentNode instanceof index_ts_1.ExtensionMPTNode) {
            ...
            if (parentNode === undefined || parentNode instanceof index_ts_1.ExtensionMPTNode) {
                this.root(this.EMPTY_TRIE_ROOT);
                return;
            }
            if (!(parentNode instanceof index_ts_1.BranchMPTNode)) {
            ...
            try {
                await this.walkTrie(this.root(), async (_, node, currentKeyNibbles, walkController) => {
                    ...
                    if (node instanceof index_ts_1.ExtensionMPTNode) {
                        ...
                        if (!(0, index_ts_1.isRawMPTNode)(node.value()) && (0, util_1.bytesToUnprefixedHex)(node.value()) === dbKeyHex) {
                            found = true;
                            return;
                        }
                        walkController.allChildren(node, currentKeyNibbles);
