...
export function valuesArrayToHeaderData(values) {
    const [parentHash, uncleHash, coinbase, stateRoot, transactionsTrie, receiptTrie, logsBloom, difficulty, number, gasLimit, gasUsed, timestamp, extraData, mixHash, nonce, baseFeePerGas, withdrawalsRoot, blobGasUsed, excessBlobGas, parentBeaconBlockRoot, requestsHash, blockAccessListHash, slotNumber,] = values;
    if (values.length > 23) {
        throw EthereumJSErrorWithoutCode(`invalid header. More values than expected were received. Max: 23, got: ${values.length}`);
    }
    if (values.length < 15) {
